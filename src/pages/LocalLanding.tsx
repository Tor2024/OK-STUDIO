...
export default function LocalLanding() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="pb-24">
      ...
      <header className="bg-[#141414] text-[#F1F3EA] py-20 md:py-32 border-b border-[#C5C5C5] relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto p-6 md:p-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-20">
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {[
                'Lokale Expertise & Markтkenntnis',
                'Persönliche Beratung vor Ort',
                'Hochperformante Web-Lösungen',
                'Messbare Ergebnisse & Leads'
              ].map(item => (
                <div key={item} className="flex items-center gap-3 font-mono text-sm border-b border-[#C5C5C5] pb-2">
                  <CheckCircle2 size={16} className="text-[#616752]" />
                  <span>{item}</span>
                </div>
              ))}
            </div>
          </div>
          <div className="bg-[#F1F3EA] border border-[#C5C5C5] p-8">
            <span className="telemetry-label block mb-4">DIREKTKONTAKT</span>
            <p className="font-serif italic text-lg mb-6 opacity-60">Lassen Sie uns Ihr Projekt in {landing.city} gemeinsam zum Erfolg führen.</p>
            <Link to="/contact" className="flex items-center justify-between bg-[#616752] text-white p-4 font-mono text-xs tracking-widest hover:opacity-90 transition-all">
        ...
        <div className="mt-20 pt-12 border-t border-[#C5C5C5] text-center">
          <span className="telemetry-label block mb-4 opacity-30">PROJEKT_STARTEN</span>
          <h3 className="font-display font-black text-3xl uppercase mb-8">Bereit für den digitalen Relaunch?</h3>
          <Link to="/contact" className="inline-block bg-[#616752] text-white px-12 py-5 font-mono text-xs tracking-[0.3em] hover:scale-105 transition-transform shadow-xl">
      ...
      <style>{`
        .insight-md h2 { font-family: var(--font-display); font-weight: 900; text-transform: uppercase; font-size: 2rem; margin: 3rem 0 1.5rem; border-bottom: 1px solid #C5C5C5; padding-bottom: 0.5rem; color: #616752; }
        .insight-md p { font-family: var(--font-serif); font-size: 1.125rem; line-height: 1.8; margin-bottom: 1.5rem; opacity: 0.8; }
        .insight-md ul { margin-bottom: 1.5rem; padding-left: 1.5rem; list-style-type: disc; }
        .insight-md li { margin-bottom: 0.5rem; font-family: var(--font-mono); font-size: 0.9rem; }
      `}</style>
    </motion.div>
  );
}
